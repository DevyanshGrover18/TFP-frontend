...
function buildVariantOptions(product) {
  ...
  const fromVariants = product.variants.map((v, i) => ({
    id: `${v.name}-${i}`,
...
const ProductDetail = ({ product }) => {
  ...
  const [activeVariantImages, setActiveVariantImages] = useState(
    product.media?.gallery ?? []
  );
    ...
    <div
      ...
      <div className="mx-auto max-w-[1260px] px-4 sm:px-6 pb-24">
        ...
        <nav
          ...
          <span style={{ color: "#f0e0dc" }}>/</span>
          <Link to={`/products?subsubcategory=${product.subSubCategoryId.id}`}
            style={{ color: "#d94f4f" }}>
